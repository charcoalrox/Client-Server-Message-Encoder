...
int main(int argc, char *argv[]) {
  ...
  char *outputString = (char *)malloc((fileSize + fileSize2 + 3) * sizeof(char)); //must be malloc'd or else my code will crash with large enough keys
  outputString[0] = '\0'; //strcat looks for null terminator when deciding end of string
  strcat(outputString, inputString);
  strcat(outputString, "1"); //for strtoking in the decoder but I've made it a 1 so it's also a flag that this output comes from an encoder. Decoder has a 2
